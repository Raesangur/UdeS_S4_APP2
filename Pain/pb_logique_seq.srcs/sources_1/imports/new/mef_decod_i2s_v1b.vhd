...
architecture Behavioral of mef_decod_i2s_v1b is
   ...
   type mef_m1_states is (
    ...
    signal m1_current_state, m1_next_state : mef_m1_states;
...
-- trouver prochain �tat
process(i_lrc, i_cpt_bits, m1_current_state)
begin
    case(m1_current_state) is
        when init =>
            if (i_lrc='0') then
               m1_next_state <= gauche;
            end if; 
        when gauche =>
            if (i_cpt_bits(4)='1') AND (i_cpt_bits(3)='1') then --  (i_cpt_bits(2 downto 0)="111") then
               -- compteur � 23
               m1_next_state <= fin_gauche;
            end if; 
        when fin_gauche =>
            if (i_lrc='0') then
                m1_next_state <= transition;
            else
                -- �vite de perdre un bit
                m1_next_state <= droite;
            end if; 
        when transition =>
            if (i_lrc='1') then
               m1_next_state <= droite;
...
-- conversion de l'�tat pour trouver les sorties
process(m1_current_state)
begin
    case m1_current_state is
        when init =>    
            o_bit_enable    <= '0';
